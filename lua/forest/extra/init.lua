...
function M.setup()
  ...
  for extra, info in pairs(M.extras) do
    package.loaded["forest.extra." .. extra] = nil
    local plugin = require("forest.extra." .. extra)
    for style, style_name in pairs(styles) do
      config.setup({ style = style })
      local colors = require("forest.colors").setup({ transform = true })
      local fname = extra .. "/forest_" .. style .. "." .. info.ext
      colors["_upstream_url"] = "https://github.com/folke/forest.nvim/raw/main/extras/" .. fname
