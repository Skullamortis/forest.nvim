...
# A Big shoutout and thanks to Folke and his works for the Vim/Neovim community!
...
- Provides [forest](https://github.com/folke/forest.nvim)
...
> ❗️ Set the configuration **BEFORE** loading the color scheme with `colorscheme forest`.

The theme offers four styles: [serene](#serene), [night](#night),
and [day](#day).

The [day](#day) style is used when `{ style = "day" }` is passed to
`setup(options)` or when `vim.o.background = "light"`.

[forest](https://github.com/folke/forest.nvim) uses the default options,
...
```lua
local colors = require("forest.colors").setup() -- pass in any of the config options as explained above
...
1. Create a file like `lua/forest/extra/cool-app.lua`.
2. Add the name and output file extension to the `extras` table in
   `lua/forest/extra/init.lua`.
3. Run the following command to generate new [extra](#-extras) themes:

   ```sh
   nvim --headless "+lua require('forest.extra').setup()" +qa
   ```
